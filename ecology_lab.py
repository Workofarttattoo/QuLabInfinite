...
class EcologyLab:
    ...
    def habitat_fragmentation_analysis(self,
        ...
        # Calculate perimeter using global mask erosion
        habitat_mask = landscape_matrix > 0
        eroded_habitat = ndimage.binary_erosion(habitat_mask)
        edge_cells = habitat_mask & ~eroded_habitat
        patch_perimeters = np.bincount(labeled_patches[edge_cells], minlength=n_patches+1)[1:]
        ...
        if n_patches > 1:
            ...
            centroids = np.array([c for c in centroids if not np.isnan(c).any()])
