...
class EcologyLab:
    ...
    def habitat_fragmentation_analysis(self,
        ...
        connectivity = 0
        if n_patches > 1:
            # Calculate distances between patch centroids using cKDTree
            # Suppress RuntimeWarning for empty patches (division by zero)
            import warnings
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", category=RuntimeWarning)
                centroids_raw = ndimage.center_of_mass(np.ones_like(labeled_patches), labeled_patches, index=np.arange(1, n_patches + 1))

            # Filter out NaNs from empty patches
            centroids = np.array([c for c in centroids_raw if not np.isnan(c[0])])

            if len(centroids) > 1:
                # Use cKDTree for O(N log N) distance queries instead of O(N^2) distance_matrix
