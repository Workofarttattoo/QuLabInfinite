...
class EcologyLab:
    ...
    def habitat_fragmentation_analysis(self,
        ...
        # Patch metrics (Vectorized globally using bincount and binary_erosion)
        if n_patches > 0:
        ...
        patch_sizes = np.array(patch_sizes, dtype=float) * cell_size ** 2  # Convert to area
        patch_perimeters = np.array(patch_perimeters, dtype=float) * cell_size

        # Fragmentation metrics (cast to Python float to ensure JSON serializability)
        ...
        edge_density = total_edge / (total_cells * cell_size ** 2) if total_cells > 0 else 0.0
        ...
        for size, perimeter in zip(patch_sizes, patch_perimeters, strict=False):
            if size > 0:
                ...
                shape_index = perimeter / expected_perimeter if expected_perimeter > 0 else 1.0
                shape_indices.append(shape_index)

        mean_shape_index = float(np.mean(shape_indices)) if shape_indices else 1.0

        # Connectivity (proportion of habitat within distance threshold)
        distance_threshold = 3  # cells
        connectivity = 0.0
        if n_patches > 1:
            # Calculate distances between patch centroids (Vectorized with cKDTree and center_of_mass)
            centroids = np.array(ndimage.center_of_mass(landscape_matrix, labeled_patches, index=np.arange(1, n_patches + 1)))
            tree = spatial.cKDTree(centroids)
            # Offset distance_threshold to match `<` condition exactly
