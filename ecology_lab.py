...
class EcologyLab:
    ...
    def habitat_fragmentation_analysis(self,
        ...
        connectivity = 0
        if n_patches > 1:
            # Optimize centroid calculation using ndimage.center_of_mass instead of slow looping
            centroids = np.array(ndimage.center_of_mass(
                np.ones_like(labeled_patches),
                labeled_patches,
                index=np.arange(1, n_patches + 1)
            ))

            # Replace O(N^2) distance_matrix with O(N log N) cKDTree query_pairs
            tree = spatial.cKDTree(centroids)
            # query_pairs gives unique pairs <= threshold. Multiply by 2 to match distance_matrix < threshold
            # offset by 1e-9 to mimic strict less-than
            connected_pairs = 2 * len(tree.query_pairs(distance_threshold - 1e-9))
