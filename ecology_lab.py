...
class EcologyLab:
    ...
    def habitat_fragmentation_analysis(self,
        ...
        # Patch metrics
        if n_patches > 0:
            # Vectorized metrics using numpy.bincount (global instead of per-patch loops)
            patch_sizes = np.bincount(labeled_patches.ravel())[1:n_patches + 1]

            # Global binary erosion for all edges simultaneously
            eroded = ndimage.binary_erosion(landscape_matrix == 1)
            edges = (landscape_matrix == 1) & ~eroded
            edge_labels = labeled_patches[edges]
            patch_perimeters = np.bincount(edge_labels, minlength=n_patches + 1)[1:n_patches + 1]
