...
class EcologyLab:
    ...
    def habitat_fragmentation_analysis(self,
        ...
        # Patch metrics - Vectorized globally
        if n_patches > 0:
            patch_sizes_raw = np.bincount(labeled_patches.ravel())[1:n_patches + 1]

            # Global perimeter calculation
            eroded_global = ndimage.binary_erosion(landscape_matrix == 1)
            edge_cells = (landscape_matrix == 1) & ~eroded_global
            edge_labels = labeled_patches[edge_cells]
            patch_perimeters_raw = np.bincount(edge_labels, minlength=n_patches + 1)[1:n_patches + 1]
        else:
            patch_sizes_raw = np.array([])
        ...
        if len(patch_sizes) > 0:
            ...
            valid_indices = (expected_perimeters > 0) & (patch_sizes > 0)
            shape_indices = np.ones_like(patch_sizes, dtype=float)
            shape_indices[valid_indices] = patch_perimeters[valid_indices] / expected_perimeters[valid_indices]
            mean_shape_index = float(np.mean(shape_indices))
        else:
        ...
        if n_patches > 1:
            ...
            centroids = np.array([c for c in centroids if c is not None and not np.isnan(c[0])])

            if len(centroids) > 1:
                tree = spatial.cKDTree(centroids)
                # query_pairs gives unique pairs (i, j) where distance <= threshold
                # we offset threshold slightly to match strictly less than (<)
                pairs = tree.query_pairs(distance_threshold - 1e-9)
                connected_pairs = len(pairs) * 2
                possible_pairs = n_patches * (n_patches - 1)
                connectivity = float(connected_pairs / possible_pairs) if possible_pairs > 0 else 0.0
