...
class EcologyLab:
    ...
    def habitat_fragmentation_analysis(self,
        ...
        # ⚡ Bolt: Vectorize patch metrics calculations and spatial distance operations
        # Compute sizes and perimeters globally instead of per-patch looping
        ...
        edges_global = habitat_mask & ~eroded_global
        patch_perimeters_raw = np.bincount(labeled_patches.ravel(), weights=edges_global.ravel())[1:]
        ...
        valid_sizes = patch_sizes > 0
        expected_perimeters = 2 * np.sqrt(np.pi * patch_sizes)
        shape_indices = np.ones_like(patch_sizes, dtype=float)
        valid_mask = valid_sizes & (expected_perimeters > 0)
        shape_indices[valid_mask] = patch_perimeters[valid_mask] / expected_perimeters[valid_mask]
        ...
        connectivity = 0.0
        if n_patches > 1:
            # Replace O(N^2) distance matrix with cKDTree and vectorize centroid calculation
            indices = np.arange(1, n_patches + 1)
            centroids = np.array(ndimage.center_of_mass(habitat_mask, labeled_patches, index=indices))
            tree = spatial.cKDTree(centroids)
            connected_pairs_count = len(tree.query_pairs(distance_threshold - 1e-9)) * 2
            possible_pairs = n_patches * (n_patches - 1)
            connectivity = float(connected_pairs_count / possible_pairs) if possible_pairs > 0 else 0.0
