...
class EcologyLab:
    ...
    def habitat_fragmentation_analysis(self,
        ...
        if n_patches > 0:
            ...
            # Compute edge cells globally for O(N) perimeters
            eroded_all = ndimage.binary_erosion(landscape_matrix == 1)
            edges = (landscape_matrix == 1) & ~eroded_all
            edge_counts = np.bincount(labeled_patches[edges])
            patch_perimeters = np.zeros(n_patches)
            if len(edge_counts) > 1:
                patch_perimeters[:len(edge_counts)-1] = edge_counts[1:]
        else:
        ...
        if len(patch_sizes) > 0:
            expected_perimeter = 2 * np.sqrt(np.pi * patch_sizes)
            valid = expected_perimeter > 0
            shape_indices = np.ones_like(patch_sizes, dtype=float)
            shape_indices[valid] = patch_perimeters[valid] / expected_perimeter[valid]
        ...
        if n_patches > 1:
            ...
            centroids = ndimage.center_of_mass(landscape_matrix, labeled_patches, index=np.arange(1, n_patches + 1))

            from scipy.spatial import cKDTree
            # Use cKDTree instead of O(N^2) distance_matrix to avoid OOM and speed up
        ...
        return {
            ...
            'fragmentation_index': float(1 - largest_patch_index),  # Higher = more fragmented
