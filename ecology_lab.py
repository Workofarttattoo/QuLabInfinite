...
class EcologyLab:
    ...
    def habitat_fragmentation_analysis(self,
        ...
        # Patch metrics
        # Vectorized calculation for patch sizes
        patch_sizes = np.bincount(labeled_patches.flat)[1:]

        # Vectorized calculation for patch perimeters
        eroded_all = ndimage.binary_erosion(landscape_matrix == 1)
        perimeter_mask = (landscape_matrix == 1) & ~eroded_all
        patch_perimeters = np.bincount(labeled_patches[perimeter_mask], minlength=n_patches+1)[1:]
        ...
        expected_perimeters = 2 * np.sqrt(np.pi * patch_sizes[valid_sizes])
        shape_indices = np.where(expected_perimeters > 0,
                                 patch_perimeters[valid_sizes] / expected_perimeters,
                                 1)
        ...
        connectivity = 0.0
        if n_patches > 1:
            # Calculate distances between patch centroids using cKDTree instead of O(N^2) distance matrix
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", category=RuntimeWarning)
                centroids = np.array(ndimage.center_of_mass(
                    np.ones_like(landscape_matrix),
                    labeled_patches,
            ...
            if len(valid_centroids) > 1:
                ...
                # Query pairs within distance, multiply by 2 for bidirectional exact parity with previous matrix sum
