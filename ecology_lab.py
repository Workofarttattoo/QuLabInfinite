...
class EcologyLab:
    ...
    def habitat_fragmentation_analysis(self,
        ...
        # Patch metrics
        # Vectorized patch metrics calculation
        patch_sizes_raw = np.bincount(labeled_patches.ravel())[1:]
        patch_sizes = patch_sizes_raw * cell_size ** 2

        # Calculate perimeter globally
        eroded_all = ndimage.binary_erosion(landscape_matrix == 1)
        edge_mask = (landscape_matrix == 1) & ~eroded_all
        edge_counts = np.bincount(labeled_patches[edge_mask])

        # Ensure array is size n_patches by padding if max patch ID wasn't in edge_mask
        if len(edge_counts) <= n_patches:
            edge_counts = np.pad(edge_counts, (0, n_patches + 1 - len(edge_counts)))
        patch_perimeters = edge_counts[1:] * cell_size
        ...
        if n_patches > 1:
            ...
            # Filter out NaNs if any patch is empty
            valid_centroids = centroids[~np.isnan(centroids[:, 0])]
