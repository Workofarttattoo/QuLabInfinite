...
class EcologyLab:
    ...
    def habitat_fragmentation_analysis(self,
        ...
        if n_patches == 0:
            return {
                'habitat_proportion': habitat_proportion, 'number_of_patches': n_patches,
                'mean_patch_size': 0, 'largest_patch_index': 0, 'edge_density': 0,
                'total_edge_length': 0, 'mean_shape_index': 1, 'connectivity': 0,
                'fragmentation_index': 1, 'patch_density': 0
            }

        # Optimization: Global vectorization
        patch_sizes_raw = np.bincount(labeled_patches.ravel(), minlength=n_patches + 1)[1:]
        ...
        patch_perimeters_raw = np.bincount(labeled_patches[edge_cells], minlength=n_patches + 1)[1:]
        ...
        shape_indices[valid] = patch_perimeters[valid] / expected_perimeters[valid]
