...
class EcologyLab:
    ...
    def habitat_fragmentation_analysis(self,
        ...
        # Vectorized patch size calculation
        ...
        edge_matrix = (landscape_matrix == 1) & ~eroded_matrix
        edge_cells_per_patch = np.bincount(labeled_patches[edge_matrix])[1:] if np.any(edge_matrix) else np.zeros(n_patches)
        patch_perimeters = edge_cells_per_patch * cell_size
        ...
        shape_indices = np.where(expected_perimeters > 0, patch_perimeters / expected_perimeters, 1.0)
        valid_mask = patch_sizes > 0
        mean_shape_index = np.mean(shape_indices[valid_mask]) if np.any(valid_mask) else 1.0
        ...
        if n_patches > 1:
            ...
            # Vectorized distance calculation with cKDTree
            if len(centroids) > 1:
