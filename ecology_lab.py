...
class EcologyLab:
    ...
    def habitat_fragmentation_analysis(self,
        ...
        # Fast patch metrics using bincount and global erosion
        if n_patches > 0:
            patch_sizes = np.bincount(labeled_patches.ravel())[1:] * cell_size ** 2

            # Fast perimeter calculation
            eroded_matrix = ndimage.binary_erosion(landscape_matrix == 1)
            perimeter_matrix = (landscape_matrix == 1) & ~eroded_matrix
            patch_perimeters = np.bincount(labeled_patches.ravel(), weights=perimeter_matrix.ravel())[1:] * cell_size
        else:
        ...
        if len(patch_sizes) > 0:
            valid_mask = patch_sizes > 0
            shape_indices = np.ones_like(patch_sizes, dtype=float)
            if np.any(valid_mask):
                expected_perimeters = 2 * np.sqrt(np.pi * patch_sizes[valid_mask])
                shape_indices[valid_mask] = np.where(
                    expected_perimeters > 0,
                    patch_perimeters[valid_mask] / expected_perimeters,
                    1
                )
        ...
        connectivity = 0
        if n_patches > 1:
            # Fast centroid calculation and KDTree for distances
