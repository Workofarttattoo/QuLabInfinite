...
class EcologyLab:
    ...
    def habitat_fragmentation_analysis(self,
        ...
        # Patch metrics
        # Optimization: Replace per-patch loops with global vectorization using np.bincount and global erosion
        # Calculate sizes
        patch_sizes = np.bincount(labeled_patches.ravel())[1:]

        # Calculate perimeters using global erosion
        habitat_mask = labeled_patches > 0
        eroded_global = ndimage.binary_erosion(habitat_mask)
        edge_mask = habitat_mask & ~eroded_global

        # Count edge pixels per patch
        ...
        if n_patches > 1:
            ...
            # Compute center of mass using ndimage directly on the labeled matrix
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", category=RuntimeWarning)
                centroids = ndimage.center_of_mass(
                    landscape_matrix, labeled_patches, index=np.arange(1, n_patches + 1)
                )
            # Handle cases where patches might be empty or invalid by filtering Nans/inf
            ...
            if len(valid_centroids) > 1:
                # Optimization: Replace O(N^2) distance_matrix with O(N log N) cKDTree
                # We multiply by 2 because query_pairs only counts undirected pairs (i, j where i < j)
                # distance_matrix counts both (i, j) and (j, i). Note query_pairs is <= so we offset threshold slightly.
                tree = spatial.cKDTree(valid_centroids)
                unique_pairs = tree.query_pairs(distance_threshold - 1e-9)
                connected_pairs = len(unique_pairs) * 2
            else:
                connected_pairs = 0
