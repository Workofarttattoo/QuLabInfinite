...
class ComputerVisionLab:
    ...
    def convolution2d(self, image: np.ndarray, kernel: np.ndarray,
        ...
        # Use sliding_window_view for fast vectorized convolution
        windows = np.lib.stride_tricks.sliding_window_view(image, (kh, kw))
        if stride > 1:
            windows = windows[::stride, ::stride]

        output = np.tensordot(windows, kernel, axes=([2, 3], [0, 1])).astype(np.float64)
        return output
    ...
    def max_pooling2d(self, image: np.ndarray, pool_size: int = 2,
        ...
        # Use sliding_window_view for fast vectorized pooling
        windows = np.lib.stride_tricks.sliding_window_view(image, (pool_size, pool_size))
        if stride > 1:
            windows = windows[::stride, ::stride]

        output = windows.max(axis=(2, 3)).astype(np.float64)
        return output
    ...
    def average_pooling2d(self, image: np.ndarray, pool_size: int = 2,
        ...
        output = windows.mean(axis=(2, 3)).astype(np.float64)
        return output
