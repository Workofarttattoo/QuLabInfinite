...
## 2025-03-05 - Vectorizing Computer Vision Sliding Windows
**Learning:** Pure Python nested loops over NumPy arrays for operations like convolutions and pooling introduce immense overhead.
**Action:** Always prefer `np.lib.stride_tricks.sliding_window_view` combined with vectorized operations like `np.tensordot` (for convolutions) and aggregation along window axes (e.g., `.max(axis=(2, 3))`) to dramatically accelerate matrix sliding window computations.
