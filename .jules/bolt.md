...
## 2024-06-23 - Precomputing constant matrix multiplications in Gradient Descent
**Learning:** In optimization algorithms like Gradient Descent, recomputing `X.T.dot(X.dot(theta) - y)` inside the epoch loop results in unnecessary `O(N*M)` matrix multiplications per iteration.
**Action:** Always precompute `XT_X = X.T.dot(X)` and `XT_y = X.T.dot(y)` before the loop and compute the gradient as `(XT_X.dot(theta) - XT_y)`. This changes the per-iteration complexity to `O(M^2)`, leading to roughly a ~20x speedup for typical datasets where `N > M`.
