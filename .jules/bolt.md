...
## 2025-06-24 - Coordinate Descent Residual Computation
**Learning:** In coordinate descent algorithms (like LASSO), recomputing the full residual `y - X.dot(theta)` from scratch inside the inner loop is O(n_samples * n_features).
**Action:** Always maintain the residual incrementally (`residual += X_j * old_theta_j` and `residual -= X_j * new_theta_j`). Precomputing norms also avoids repeated vector multiplications, yielding a 10x+ speedup.
