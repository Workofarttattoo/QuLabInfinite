...
## 2025-03-05 - Vectorizing Sliding-Window Operations in NumPy
**Learning:** Explicit pure Python nested loops over NumPy arrays cause massive hidden overhead (O(N^3) bottlenecks) in sliding-window operations like convolutions and pooling.
**Action:** Always prefer `np.lib.stride_tricks.sliding_window_view` combined with vectorized operations like `np.tensordot` or axis aggregations (e.g., `.max(axis=(2, 3))`) to keep operations entirely in C and drastically improve performance.
