...
## 2024-06-19 - Precomputing Constant Matrix Multiplications
**Learning:** In optimization algorithms (like Gradient Descent), matrix multiplications involving constants like feature matrices and targets (e.g., `X.T.dot(X)` and `X.T.dot(y)`) should never be inside the epoch loop. Evaluating them redundantly on every iteration dramatically slows down execution.
**Action:** Always precalculate constant matrix products before entering training/epoch loops. Replace `X.T.dot(y_pred - y)` inside the loop with a precomputed `(XtX.dot(theta) - Xty)` where `XtX` and `Xty` are constants evaluated beforehand.
