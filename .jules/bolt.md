...
## 2025-05-24 - Coordinate Descent Residual Optimization
**Learning:** Recomputing the entire residual vector (`y - X.dot(theta)`) inside the inner loop of coordinate descent algorithms (like LASSO and Elastic Net) introduces a massive O(N) bottleneck, causing the total epoch complexity to balloon to O(N * M) instead of O(M).
**Action:** Precompute the initial residual outside the epoch loop and maintain it incrementally by adding back the contribution of the old coefficient and subtracting the new coefficient. Additionally, precompute feature squared norms outside the loop to avoid redundant calculations.
