...
## 2025-05-18 - Vectorized sliding window operations
**Learning:** Native Python nested loops over NumPy arrays create hidden O(N^3) bottlenecks due to Python overhead, especially when performing slice sums inside the loop.
**Action:** Always prefer vectorized broadcasting or operations like `np.lib.stride_tricks.sliding_window_view` combined with `np.tensordot` or axis aggregations (e.g., `.max(axis=(2, 3))`) to keep operations entirely in C.
