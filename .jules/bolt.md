...
## 2024-03-24 - [Physics Engine Vectorization]
**Learning:** In LBM implementations (like `FluidDynamicsEngine`), python `for` loops over lattice directions (`self.q`) are a significant bottleneck due to interpreter overhead, especially when combined with large grid arrays in operations like equilibrium computation and Guo forcing.
**Action:** Always fully vectorize operations over the spatial grid and lattice directions simultaneously using `np.tensordot` and broadcasting (e.g., `np.tensordot(u, c, axes=(-1, 1))`). This avoids loop overhead and temporary large array allocations, yielding ~2x performance gains per function.
