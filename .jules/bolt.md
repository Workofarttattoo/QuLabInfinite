...
## 2025-02-12 - Gradient Descent Loop Redundancy
**Learning:** In machine learning algorithms like Gradient Descent, computing `X.T.dot(X.dot(theta) - y)` recalculates `X.T.dot(X)` constantly.
**Action:** Precomputing `X.T.dot(X)` and `X.T.dot(y)` outside the loop drastically improves performance.
