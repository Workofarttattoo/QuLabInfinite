...
## 2025-05-23 - Python Iteration Overhead in Vector Math
**Learning:** Using Python list comprehensions and iterative distance calculations (`np.linalg.norm` in a loop) for calculating nutrient access over thousands of cells and vessels causes a significant performance hit.
**Action:** Pre-convert the static list to a numpy array outside the loop and use vectorized evaluation (`np.min(np.linalg.norm(cell.position - vessels_array, axis=1))`) inside the loop to drastically cut overhead.
