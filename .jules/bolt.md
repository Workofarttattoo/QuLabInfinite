...
## 2025-05-18 - Sliding Window Vectorization for NumPy Performance
**Learning:** I discovered that native Python nested loops over NumPy arrays create hidden O(N^3) bottlenecks due to Python overhead, especially for image convolutions and pooling. Using np.lib.stride_tricks.sliding_window_view followed by np.tensordot or axis aggregations significantly eliminates this overhead.
**Action:** Always prefer vectorized sliding_window_view and broadcasting over manual loops for window-based operations over multi-dimensional NumPy arrays.
