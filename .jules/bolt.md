...
## 2023-10-27 - Vectorizing Evolutionary Optimization Entities
**Learning:** In optimization metaheuristics (e.g., Particle Swarm Optimization), explicit `for` loops in Python that iterate over individual population members or particles are slow.
**Action:** Vectorize entity states into dense NumPy arrays and compute epoch updates universally using batched operations, array clipping (`np.clip`), and boolean mask slicing (e.g., `scores < personal_best_scores`) to significantly reduce execution overhead.
