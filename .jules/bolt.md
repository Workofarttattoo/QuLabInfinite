...
## 2025-10-24 - Sliding Window Operations Optimization
**Learning:** Native Python nested loops over NumPy arrays for sliding-window operations (like convolutions and pooling) create hidden O(N^3) bottlenecks due to massive overhead.
**Action:** Use `np.lib.stride_tricks.sliding_window_view` combined with vectorized operations like `np.tensordot` or axis aggregations (e.g., `.max(axis=(2, 3))`) to keep operations entirely in C.
