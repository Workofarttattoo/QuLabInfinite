...
**Learning:** Sequential implementations of NMS using Python loops to calculate IoU one by one are O(n^2) and extremely slow for large numbers of bounding boxes (like 5000+).
**Action:** Always implement fully vectorized NumPy operations (e.g., using `np.maximum`/`np.minimum` over arrays) for NMS to compute intersections across bounding box arrays simultaneously. This gives a massive performance improvement (~70x faster).
