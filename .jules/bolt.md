...
## 2025-03-09 - [PSO Loop Optimization]
**Learning:** In optimization metaheuristics (e.g., Particle Swarm Optimization), avoid explicit `for` loops in Python that iterate over individual population members or particles. Instead, vectorize entity states into dense NumPy arrays and compute epoch updates universally using batched operations, array clipping (`np.clip`), and boolean mask slicing (e.g., `scores < personal_best_scores`) to significantly reduce execution overhead.
**Action:** Always attempt to convert per-entity iterations into vectorized NumPy tensor operations for continuous space algorithms in order to eliminate O(N) loop overhead.
