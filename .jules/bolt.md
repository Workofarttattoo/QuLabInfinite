...
## 2025-05-24 - Unnecessary Matrix Multiplication in Coordinate Descent
**Learning:** In Coordinate Descent algorithms, computing the full residual `y - X.dot(theta)` from scratch inside the inner feature loop leads to massive performance degradation (O(n_features * n_samples) per coordinate).
**Action:** Always precompute the initial residual and maintain it incrementally: `residual -= X[:, j] * (new_theta_j - old_theta_j)`. Additionally, precompute `X_norms_sq` outside the loops to avoid repeated vector dot products.
