...
## 2025-03-09 - Vectorize Computer Vision NMS
**Learning:** The `non_max_suppression` method in `computer_vision_lab.py` used an explicit Python loop to compute Intersection over Union (IoU) sequentially for bounding boxes, leading to an O(n^2) bottleneck. Vectorized NumPy operations using `np.maximum`/`np.minimum` can compute intersections across all bounding box arrays simultaneously.
**Action:** When implementing bounding box operations or algorithms requiring pairwise comparisons, avoid explicit Python loops and use vectorized NumPy methods to achieve significant performance improvements.
