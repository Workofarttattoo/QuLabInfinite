...
## 2025-05-22 - Vectorizing Sliding Window Operations
**Learning:** Explicit pure Python nested loops over NumPy arrays for operations like convolution or pooling introduce massive hidden O(N^3) bottlenecks due to Python iteration overhead.
**Action:** When refactoring sliding-window image operations over NumPy arrays, replace explicit loops with `np.lib.stride_tricks.sliding_window_view` to create virtual window arrays combined with vectorized operations like `np.tensordot` or axis aggregations.
