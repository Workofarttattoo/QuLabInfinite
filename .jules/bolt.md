...
## 2025-05-15 - Vectorizing Sliding Window Image Operations
**Learning:** For sliding-window image operations over NumPy arrays (such as convolutions and pooling), avoid explicit pure Python nested loops as they introduce massive overhead.
**Action:** Instead, prefer `np.lib.stride_tricks.sliding_window_view` to create virtual window arrays combined with vectorized operations like `np.tensordot` or axis aggregations (e.g., `.max(axis=(2, 3))`) for significant performance improvements.
