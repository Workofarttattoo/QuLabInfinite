...
## 2025-05-20 - Vectorized Electrostatic Optimization
**Learning:** O(N^2) loops in Python for pairwise pairwise interactions (like electrostatic energy calculation or VDW calculations) create massive overhead for large molecules.
**Action:** Replace nested loops in computational chemistry algorithms with numpy broadcasting (`np.newaxis`) and `np.triu_indices` for significant (~20x) speed improvements by leveraging C-level array operations.
