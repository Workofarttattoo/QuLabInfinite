...
## 2023-10-24 - [Vectorized Sliding Window Operations]
**Learning:** Explicit Python nested loops over NumPy arrays for sliding-window operations (like convolutions and pooling) introduce massive overhead, creating O(N^3) bottlenecks. Using np.lib.stride_tricks.as_strided combined with vectorized aggregations keeps operations entirely in C.
**Action:** Always prefer np.lib.stride_tricks.as_strided and vectorized reductions (like tensordot or max along axes) over pure Python nested loops for any sliding window image processing.
