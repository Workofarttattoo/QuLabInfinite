...
## 2025-05-23 - Vectorizing Bounding Box IoU in Non-Maximum Suppression
**Learning:** Sequential python loops that calculate intersection over union for array combinations in Non-Maximum Suppression (NMS) are incredibly slow (O(N^2)). By vectorizing these calculations using precomputed area arrays alongside `np.maximum` and `np.minimum`, we can operate on entire arrays at once and achieve >100x speedups.
**Action:** When implementing or optimizing computer vision models like NMS, never calculate bounding box overlaps sequentially in Python loops. Always use vectorized NumPy operations across bounding box coordinate arrays.
