...
## 2024-07-03 - O(1) Gradient Descent Precomputation
**Learning:** In optimization algorithms like Gradient Descent, recomputing matrix multiplications such as `y_pred = X.dot(theta)` and `X.T.dot(y_pred - y)` inside the epoch loop is extremely inefficient.
**Action:** Always precompute constant matrix multiplications like `X.T.dot(X)` and `X.T.dot(y)` outside the loop so the per-epoch calculation is reduced from O(N*F) to O(F^2).
