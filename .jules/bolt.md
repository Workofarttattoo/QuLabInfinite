...
## 2025-05-24 - Inner Loop getattr Overhead
**Learning:** Calling `getattr` repeatedly dynamically in an entity loop to retrieve field grids inside physics or medical simulation loops scales extremely poorly (O(n) per field fetch).
**Action:** When updating thousands of simulated entities, always pre-resolve data structures into static variables before iterating over the main physics update loop.
