...
## 2025-05-18 - Precomputing constants outside gradient descent loops
**Learning:** In optimization algorithms like gradient descent, repeatedly executing full feature matrix multiplications (e.g., `X.T.dot(X.dot(theta) - y)`) inside the epoch loop is a significant bottleneck. This costs $O(N \times F)$ every iteration.
**Action:** Always algebraically expand matrix operations where possible and precompute invariants (like `X.T.dot(X)` and `X.T.dot(y)`) before entering the training loop to reduce the per-epoch time complexity to $O(F^2)$.
