...
## 2026-04-06 - materials_api.py /stats caching
**Learning:** Returning stats from a large database (6.6M+ materials) without caching can cause severe performance issues. Using `TTLCache` to cache the results of expensive queries drastically improves endpoint performance without modifying database schema.
**Action:** Implemented `cachetools.TTLCache` in `materials_api.py` for the `/stats` endpoint.
