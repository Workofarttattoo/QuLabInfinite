...
## 2025-05-24 - N^2 Distance Calculations in Python Loops
**Learning:** In computational chemistry molecular mechanics, calculating O(N^2) pairwise pairwise distances (angles, VDW, electrostatic) using nested Python loops is extremely slow.
**Action:** Always vectorize pairwise distance computations using NumPy broadcasting and array masking (`np.sum((coords[:, np.newaxis, :] - coords[np.newaxis, :, :]) ** 2, axis=-1)`) to avoid Python loop overhead.
