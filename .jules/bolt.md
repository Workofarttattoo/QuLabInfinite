...
## 2025-05-24 - Gradient Descent Precomputation
**Learning:** In optimization algorithms like Gradient Descent, computing predictions (`X.dot(theta)`) and gradients (`X.T.dot(y_pred - y)`) sequentially inside the epoch loop leads to redundant calculations. The mathematical equivalence `X.T @ (X @ theta - y) = (X.T @ X) @ theta - (X.T @ y)` allows precomputing the static matrix products.
**Action:** Always precompute constant matrix multiplications outside the iterative optimization loops to reduce time complexity and achieve massive speedups.
