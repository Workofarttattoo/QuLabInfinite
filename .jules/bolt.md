...
## 2024-05-18 - Vectorize Non-Maximum Suppression (NMS)
**Learning:** Sequential, Python-loop based calculations for Intersection over Union (IoU) in NMS create serious performance bottlenecks (O(n^2)), severely slowing down computer vision labs for larger bounding box arrays. Vectorization utilizing `np.maximum`/`np.minimum` reduces this completely, enabling simultaneous computations using broadcasting.
**Action:** When implementing or optimizing NMS algorithms, always calculate intersections across bounding box arrays fully vectorized instead of sequentially in Python loops.
