...
## 2025-05-23 - Sequential Loop Bottleneck in NMS
**Learning:** Sequential Python `while`/`for` loops computing intersections and IoU array elements one at a time for Non-Maximum Suppression (NMS) creates a massive O(n²) performance bottleneck, especially on large sets of bounding boxes.
**Action:** Always replace sequential spatial overlaps like IoU loops with fully vectorized NumPy implementations (e.g., using `np.maximum`/`np.minimum` to compute all intersections simultaneously against the target box). E.g. Malisiewicz et al. method.
