...
## 2025-03-05 - Vectorizing Computer Vision
**Learning:** Native Python nested loops over NumPy arrays create huge hidden overhead, especially when computing slices in sliding-window algorithms like convolutions or pooling operations.
**Action:** Always prefer `np.lib.stride_tricks.sliding_window_view` to create a strided view, and use vectorized aggregations like `np.tensordot(..., axes=([2, 3], [0, 1]))`, `.max(axis=(2, 3))`, or `.mean(axis=(2, 3))` instead of explicit O(N^3) nested loops to eliminate overhead.
