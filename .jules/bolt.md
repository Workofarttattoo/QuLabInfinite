...
## 2025-05-23 - Vectorizing Non-Maximum Suppression (NMS)
**Learning:** Using sequential loops and individual Intersection over Union (IoU) calculations in Python for Non-Maximum Suppression causes severe O(N^2) bottlenecks when handling many bounding boxes.
**Action:** Always implement fully vectorized NumPy operations (e.g., Malisiewicz et al. method using `np.maximum` and `np.minimum` to compute intersections simultaneously) to bypass interpreter overhead and achieve >20x speedups.
