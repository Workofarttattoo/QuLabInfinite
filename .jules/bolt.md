...
## 2024-11-20 - [Vectorize repetitive cell distance calculation loops]
**Learning:** In agent-based simulations (like `tumor_simulator.py`), using Python list comprehensions and sequential `np.linalg.norm` operations inside large inner loops (such as cell-to-vessel distance calculations) causes a significant iteration bottleneck compared to vectorizing the numpy calculation once inside the loop using a pre-allocated structure.
**Action:** Always pre-convert static entities (like vessel locations) into numpy arrays outside of execution loops, and use `axis` vectorized evaluation on numpy arrays instead of list comprehensions.
