...
## 2024-05-18 - Optimize Coordinate Descent Time Complexity
**Learning:** In coordinate descent algorithms (like LASSO), computing `residual = y - X.dot(theta)` inside the innermost loop forces O(N*M) complexity per step.
**Action:** Always precompute the residual `residual = y.copy()` and incrementally maintain it inside the loop `residual -= X[:, j] * (new_theta_j - old_theta_j)`, dropping complexity to O(N). Also precompute static norms like `np.sum(X**2, axis=0)`.
