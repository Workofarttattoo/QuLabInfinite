...
## 2024-06-14 - [Optimize tumor simulator vessel distance calculation]
**Learning:** In performance-critical NumPy simulation loops calculating distances between points, sequential `np.linalg.norm()` inside Python list comprehensions creates significant iteration overhead.
**Action:** Always pre-convert static object lists (like vessel points) to numpy arrays outside of cell iteration loops and compute distances with vectorized operations (`np.sum(diff * diff, axis=1)`) to avoid per-entity python iteration.
