...
## 2025-10-25 - Computational Chemistry Force Field Vectorization
**Learning:** O(N^2) Python inner loops for calculating pairwise atomic energy components (e.g. VdW, electrostatic, nuclear repulsion) are a massive bottleneck in computational chemistry modules compared to their math equivalents.
**Action:** Always pull target data into temporary NumPy arrays (like using `np.triu_indices` for unique combinations and `np.linalg.norm(coords[:, np.newaxis, :] - coords[np.newaxis, :, :], axis=2)` for distance matrices) and calculate the energies vectorized via advanced index slices and broadcasting to yield 20-50x speedups.
