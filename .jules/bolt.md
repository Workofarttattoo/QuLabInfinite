...
## 2025-05-24 - Vectorizing Metaheuristics (Particle Swarm Optimization)
**Learning:** When vectorizing iterative metaheuristic algorithms like Particle Swarm Optimization to remove explicit `for` loops, the state update logic shifts from asynchronous (where entities can react to peers' updates within the same epoch) to synchronous (where all entities update simultaneously based on the prior epoch's state). This changes the exact search trajectory but is a mathematically valid and acceptable trade-off for massive performance gains.
**Action:** Always vectorize metaheuristics by pre-computing boundaries and applying batched np.clip and boolean mask slicing instead of looping over individuals.
