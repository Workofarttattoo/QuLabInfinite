...
## 2025-03-06 - [Vectorizing Sliding Windows]
**Learning:** Pure Python nested loops over NumPy arrays for sliding-window operations (convolutions, pooling) introduce massive overhead, hiding O(N^3) bottlenecks.
**Action:** Always prefer np.lib.stride_tricks.sliding_window_view combined with np.tensordot or axis aggregations (.max, .mean) to keep operations entirely in C, ensuring backward compatibility by casting results appropriately.
