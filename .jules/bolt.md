...
## 2025-05-15 - Vectorizing Sliding Window Operations
**Learning:** Native Python nested loops over NumPy arrays create massive O(N^3) overhead for sliding-window operations like convolutions. Explicit nested loops bottleneck performance significantly.
**Action:** Always prefer `np.lib.stride_tricks.sliding_window_view` combined with vectorized operations like `np.tensordot` or axis aggregations (e.g., `.max(axis=(2, 3))`) to keep operations entirely in C. This provides ~50x to ~270x speedups.
