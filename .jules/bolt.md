...
## 2025-07-08 - Gradient Descent O(N*M) Loop Optimization
**Learning:** In optimization loops like Gradient Descent, computing `X.T.dot(X.dot(theta) - y)` on every epoch recalculates constant matrix products unnecessarily. On tall datasets (large N, small M), this results in an $O(N \cdot M)$ operation per iteration.
**Action:** Always precompute fixed dataset matrix products such as $X^T X$ and $X^T y$ before the loop. This reduces the per-epoch gradient calculation complexity down to $O(M^2)$, resulting in massive execution time speedups (e.g., >20x on typical tall matrices).
