...
## 2025-05-08 - Synchronizing aria-current with dynamic classes
**Learning:** When dynamically toggling active states (e.g., `.active`) on interactive elements like buttons, screen readers are unaware of the visual change unless `aria-current="true"` is also updated programmatically. This ensures the component tree correctly broadcasts the "selected" status.
**Action:** Always programmatically synchronize the `aria-current` attribute when toggling custom `.active` classes on interactive elements to ensure screen reader accessibility.
