...
## 2024-06-15 - Improve prompt bar focus state
**Learning:** When styling composite input components in the frontend UI (e.g., .prompt-bar), apply interactive focus styles using the :focus-within pseudo-class on the wrapper container and explicitly set outline: none on the inner input element to ensure a cohesive, accessible focus state without double rings.
**Action:** Always check the focus styling of composite inputs to ensure clear and unified feedback without multiple conflicting focus rings.
