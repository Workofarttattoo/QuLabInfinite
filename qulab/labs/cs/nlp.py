...
class EmbeddingLayer:
    def __init__(self, config: NLPConfig):
        self.config = config
        # Initialize the embedding matrix once to prevent massive per-pass overhead
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        # Cast indices to int and safely handle Out-Of-Vocabulary tokens
        x_indices = np.clip(x.astype(int), 0, self.config.vocab_size - 1)
        return self.embedding_matrix[x_indices].astype(np.float64)
