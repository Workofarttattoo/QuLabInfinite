...
class EmbeddingLayer:
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        # Map out-of-bounds indices to a known token or clip to prevent IndexError
        x = np.clip(x.astype(int), 0, self.config.vocab_size - 1)
        return self.embedding_matrix[x].astype(np.float64)
