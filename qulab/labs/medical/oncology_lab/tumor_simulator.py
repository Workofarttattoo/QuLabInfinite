...
class TumorSimulator:
    ...
    def step(self, dt: Optional[float] = None):
        ...
        # Pre-convert reference lists into a NumPy array outside the loop
        # to vectorize distance calculations, avoiding significant Python iteration overhead
        vessel_locs = np.array(self.microenvironment.vessel_locations) if self.microenvironment.vessel_locations else np.array([])
        ...
        for cell in self.cells:
            ...
            if len(vessel_locs) > 0:
                min_distance = np.min(np.linalg.norm(vessel_locs - cell.position, axis=1))
