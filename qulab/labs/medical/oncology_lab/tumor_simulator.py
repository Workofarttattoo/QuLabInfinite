...
class TumorSimulator:
    ...
    def step(self, dt: Optional[float] = None):
        ...
        # Pre-resolve static vessel array for fast loop execution
        vessels_arr = np.array(self.microenvironment.vessel_locations) if self.microenvironment.vessel_locations else None
        ...
        for cell in self.cells:
            ...
            if vessels_arr is not None and vessels_arr.size > 0:
                # Fast distance calculation utilizing pre-converted numpy array
                distances = np.linalg.norm(cell.position - vessels_arr, axis=1)
