...
class TumorSimulator:
    ...
    def step(self, dt: Optional[float] = None):
        ...
        # Pre-compute vessel locations as numpy array for fast distance calculation
        vessels_array = np.array(self.microenvironment.vessel_locations) if self.microenvironment.vessel_locations else np.array([])
        has_vessels = len(vessels_array) > 0
