...
## 2026-02-24 - Synchronize Active States and ARIA
**Learning:** Screen readers miss active states on custom components if the visual active class is not accompanied by the semantic aria-current attribute, and placeholder-only inputs remain inaccessible.
**Action:** Always ensure UI active state toggles synchronize the active CSS class with aria-current='true', and provide explicit aria-labels for placeholder-only inputs.
