...
## 2026-05-26 - DOM ordering affects a11y attribute application
**Learning:** When generating interactive UI components like lab selection buttons, appending them to the DOM *before* applying active states via JS functions ensures that querySelectorAll successfully finds the elements to apply accessibility attributes like aria-current.
**Action:** Always append newly created elements to the document body or container before invoking state-management functions that apply visual classes and ARIA attributes.
