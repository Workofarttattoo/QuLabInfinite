#!/usr/bin/env python3
"""
CHGNet PES inference (energy/forces/stress) using MatGL (DGL backend).

Examples:
    MATGL_BACKEND=DGL python scripts/matgl_chgnet_pes.py --cif struct.cif
    MATGL_BACKEND=DGL python scripts/matgl_chgnet_pes.py --cif struct.cif \
...
def main():
    parser = argparse.ArgumentParser(description="Run CHGNet PES (DGL backend) via MatGL.")
    parser.add_argument("--cif", required=True, help="Path to CIF file.")
    parser.add_argument(
        "--model",
        default="CHGNet-MatPES-r2SCAN-2025.2.10-2.7M-PES",
        help=(
            "Pretrained model name. Examples: "
            "CHGNet-MatPES-r2SCAN-2025.2.10-2.7M-PES (default), "
