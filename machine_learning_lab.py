...
class MachineLearningLab:
    ...
    def gradient_descent(self, X: np.ndarray, y: np.ndarray,
        ...
        # ⚡ Bolt Optimization: Precompute expensive matrix multiplications X.T.dot(X) and X.T.dot(y)
        # to avoid O(N*M) calculation inside the epoch loop, reducing it to O(M^2).
        XTX = X.T.dot(X)
        XTy = X.T.dot(y)
