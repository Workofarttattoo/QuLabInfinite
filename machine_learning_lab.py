...
class MachineLearningLab:
    ...
    def gradient_descent(self, X: np.ndarray, y: np.ndarray,
        ...
        # Precompute constant matrices to avoid recomputing in the loop
        XT_X_scaled = (2/n_samples) * X.T.dot(X)
        XT_y_scaled = (2/n_samples) * X.T.dot(y)

        for epoch in range(self.config.epochs):
            # Compute gradient directly
            gradient = XT_X_scaled.dot(theta) - XT_y_scaled
